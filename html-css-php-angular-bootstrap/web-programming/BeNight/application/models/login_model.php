...
class LoginModel
{
    ...
    public function login()
    {
        ...
        $result = $sth->fetch();
	if($result->user_account_type == 1 || $result->user_account_type == 2 || $result->user_account_type == 3){
		$sth = $this->db->prepare("SELECT *
                                    FROM   users
...
$sth->execute(array(':user_name' => $_POST['user_name'], ':provider_type' => 'DEFAULT'));
	...
	$result = $sth->fetch();
	    ...
	    Session::set('user_first', $result->person_first_name);
	    Session::set('user_last', $result->person_last_name);
			//Session::set('promoter_first', $result->promoter_first_name);
	    	//Session::set('promoter_last', $result->promoter_last_name);
            Session::set('user_provider_type', 'DEFAULT');
            // put native avatar path into session
            Session::set('user_profile_path', $result->user_profile_picture);
		...
		//$cookie_string);
                // set cookie
                setcookie('rememberme', $cookie_string, time() + COOKIE_RUNTIME, COOKIE_BASE_PATH, COOKIE_DOMAIN);
            }
    ...
    public function logout()
    {
        ...
        setcookie('rememberme', false, time() - (3600 * 3650), COOKIE_BASE_PATH, COOKIE_DOMAIN);
    ...
    public function registerNewUser()
    {
		...
		if(empty($_POST['first_name']) OR empty($_POST['last_name'])){
			$_SESSION["feedback_negative"][] = FEEDBACK_USERNAME_FIELD_EMPTY;
		} elseif ($_POST['user_email'] !== $_POST['user_email_repeat']) {
            ...
            AND !empty($_POST['user_email_repeat'])
            AND !empty($_POST['user_password_new'])
            AND ($_POST['user_email'] === $_POST['user_email_repeat'])) {
			...
			*/
	    	...
	    	$sql = "INSERT INTO people (person_user_id, person_first_name, person_last_name, person_gender, person_birthday)
                    VALUES (:user_id, :person_first_name, :person_last_name, :person_gender, :person_birthday)";
            $query = $this->db->prepare($sql);
            $query->execute(array(':user_id' => $user_id,
				  ':person_first_name' => $_POST['first_name'],
                                  ':person_last_name' => $_POST['last_name'],
                                  ':person_gender' => $_POST['sex'] ,
                                  ':person_birthday' => date('Y-m-d', strtotime(str_replace('-', '/', $_POST['user_birthday'])))));
            $count =  $query->rowCount();
            if ($count != 1) {
                $_SESSION["feedback_negative"][] = FEEDBACK_ACCOUNT_CREATION_FAILED;
                return false;
			...
			$query = $this->db->prepare("SELECT * FROM users LEFT JOIN people ON people.person_user_id = users.user_id WHERE user_id = :user_id");
            $query->execute(array(':user_id' => $user_id));
            if ($query->rowCount() != 1) {
                $_SESSION["feedback_negative"][] = FEEDBACK_UNKNOWN_ERROR;
                return false;
